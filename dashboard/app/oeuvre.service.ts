...
export class OeuvreService {
  ...
  private bearUrl = 'http://localhost:8081/api/oeuvres';  // URL to web api
  ...
  getBears() {
    ...
    .map(response => <Oeuvre[]> response.json())
    // .catch(this.handleError)
    ;
  }  

  getAllPic() : Observable<Oeuvre[]>{
    // ...using get request
    return this.http.get(this.bearUrl)
  ...
  // Add a new comment
  addBear (body: Object): Observable<Oeuvre[]> {
  ...
  findBearById(id: string) : Observable<Oeuvre> {
    ...
    deleteBear(id: string) : Observable<Oeuvre> {
    // End point for list of pets:
    // http://api.petfinder.com/pet.find?key=[API_KEY]&animal=[ANIMAL]&format=json&location=texas
    const endPoint = 'get'
    // URLSearchParams makes it easier to set query parameters and construct URL
    ...
    .map((res: Response) => res.json());
     ...
     updateBear(id: string) : Observable<Oeuvre> {
    // End point for list of pets:
    // http://api.petfinder.com/pet.find?key=[API_KEY]&animal=[ANIMAL]&format=json&location=texas
    const endPoint = 'get'
    // URLSearchParams makes it easier to set query parameters and construct URL
